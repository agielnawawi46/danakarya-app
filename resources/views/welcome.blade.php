...
<head>
    ...
    <meta name="description" content="DanaKarya — Platform koperasi digital terintegrasi untuk kelola simpanan, pinjaman, dan SHU secara transparan, aman, dan modern.">
    <meta property="og:title" content="DanaKarya — Koperasi Digital Masa Depan">
    <meta property="og:description" content="Kelola simpanan, pantau aset, dan kembangkan ekonomi bersama DanaKarya — transparan, aman, dan digital.">
...
<header class="fixed top-0 left-0 right-0 z-[100] px-4 md:px-10 py-4 transition-all duration-500"
    ...
    <nav class="max-w-7xl mx-auto flex items-center justify-between" role="navigation" aria-label="Navigasi utama">
        ...
        <div class="hidden md:flex items-center gap-8">
            ...
            @auth
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="px-6 py-3 border border-slate-200 text-slate-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">
                        Keluar
                    </button>
                </form>
            @else
        ...
        <button @click="mobileOpen = !mobileOpen"
            ...
            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 6h16M4 12h16M4 18h16"/></svg>
        </button>
    </nav>
    {{-- Mobile Menu --}}
    <div x-show="mobileOpen" x-collapse x-cloak class="md:hidden mt-4 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 space-y-2">
        ...
        @auth
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full block px-4 py-3 text-xs font-black uppercase tracking-widest text-slate-600 border border-slate-200 rounded-xl text-center hover:bg-slate-50">Keluar</button>
            </form>
        @else
                ...
                <div class="space-y-10">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                        <a href="{{ route('login') }}" class="group inline-flex items-center gap-3 px-8 py-5 bg-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-blue-600 transition-all shadow-xl shadow-slate-200 active:scale-[0.97]">
                            Mulai Sekarang
                            <svg class="w-4 h-4 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                        </a>
                        <a href="#fitur" class="text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-colors flex items-center gap-2">
                            Lihat Fitur
                            <svg class="w-4 h-4 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 14l-7 7m0 0l-7-7m7 7V3"/></svg>
                        </a>
                    </div>
                </div>
                <div class="hidden lg:flex justify-center">
                    <div class="relative group">
                        <div class="w-96 h-96 bg-gradient-to-br from-slate-100 via-white to-slate-200 rounded-full rotate-3 shadow-2xl flex items-center justify-center overflow-hidden group-hover:rotate-0 transition-transform duration-700 relative">
                            <img
                                src="{{ asset('images/logo.jpg') }}"
                                alt="DanaKarya Logo"
                                class="w-full h-full object-cover relative z-0"
                            >
                            <div class="absolute inset-0 bg-gradient-to-br from-blue-600/10 via-transparent to-indigo-600/10 z-10 pointer-events-none" aria-hidden="true"></div>
                            <div class="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,_rgba(59,130,246,0.12),transparent_60%)] z-10 pointer-events-none" aria-hidden="true"></div>
                        </div>
                        <div class="absolute -bottom-6 -right-6 w-[200px] h-[168px] bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-6 flex flex-col justify-center group-hover:-translate-y-2 transition-transform duration-500">
                        ...
                        <div class="absolute top-4 left-4 w-16 h-16 bg-emerald-500 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-emerald-200 group-hover:scale-110 transition-transform duration-500">
                            <svg class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="group bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:border-blue-100 transition-all duration-500 overflow-hidden">
                    <div class="relative h-44 overflow-hidden">
                        ...
                        <div class="absolute top-4 left-4 p-2.5 bg-white/90 backdrop-blur-sm text-blue-600 rounded-xl shadow-lg group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $fitur['icon'] }}"/></svg>
                        </div>
                    </div>
                    <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        ...
                        <div class="flex items-center gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-blue-200 hover:bg-white transition-all cursor-default">
                            <div class="p-2.5 bg-white text-blue-600 rounded-xl shadow-sm group-hover:bg-blue-600 group-hover:text-white group-hover:shadow-lg group-hover:shadow-blue-200 transition-all">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $item['icon'] }}"/></svg>
                            </div>
    ...
    <section id="kontak" class="relative overflow-hidden bg-slate-900">
        ...
        <div class="max-w-6xl mx-auto px-6 py-28 md:py-36 relative z-10">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 max-w-4xl mx-auto">
                <a href="https://wa.me/6281211112222" target="_blank" rel="noopener noreferrer" class="group p-6 md:p-8 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-emerald-500/10 hover:border-emerald-400/40 transition-all duration-500 text-center">
                ...
                <a href="https://m.me/danakaryaofficial" target="_blank" rel="noopener noreferrer" class="group p-6 md:p-8 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-indigo-500/10 hover:border-indigo-400/40 transition-all duration-500 text-center">
                    <div class="flex flex-col items-center gap-3">
                        <div class="p-3.5 bg-white/10 rounded-xl text-white group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500">
                            <svg class="w-6 h-6 fill-current" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 0C5.126 0 0 4.708 0 11.23c0 3.394 1.365 6.413 3.593 8.443.187.17.293.407.288.656l-.014 2.193c-.006.915.962 1.503 1.716 1.01l2.454-1.603a.76.76 0 01.52-.102c1.104.305 2.276.47 3.443.47 6.874 0 12-4.708 12-11.23C24 4.708 18.874 0 12 0z"/></svg>
                        </div>
                ...
                <a href="https://instagram.com/danakaryaofficial" target="_blank" rel="noopener noreferrer" class="group p-6 md:p-8 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-pink-500/10 hover:border-pink-400/40 transition-all duration-500 text-center">
                    <div class="flex flex-col items-center gap-3">
                        <div class="p-3.5 bg-white/10 rounded-xl text-white group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                ...
                <a href="https://line.me/ti/p/~danakaryaofficial" target="_blank" rel="noopener noreferrer" class="group p-6 md:p-8 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-lime-500/10 hover:border-lime-400/40 transition-all duration-500 text-center">
                    <div class="flex flex-col items-center gap-3">
                        <div class="p-3.5 bg-white/10 rounded-xl text-white group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500">
                            <svg class="w-6 h-6 fill-current" viewBox="0 0 24 24" aria-hidden="true"><path d="M24 10.304c0-5.269-5.383-9.556-12-9.556s-12 4.287-12 9.556c0 4.723 4.266 8.672 10.021 9.428l-.711 2.347c-.082.271.19.508.443.342l5.161-3.39c4.276-.641 7.086-4.041 7.086-7.181z"/></svg>
                        </div>
                ...
                <a href="mailto:support@koperasi.id" class="group p-6 md:p-8 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-sky-500/10 hover:border-sky-400/40 transition-all duration-500 text-center">
                    <div class="flex flex-col items-center gap-3">
                        <div class="p-3.5 bg-white/10 rounded-xl text-white group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        </div>
...
<button x-show="showScrollTop"
    ...
    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>
</button>
